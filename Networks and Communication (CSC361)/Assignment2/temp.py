...
def parse_packet_data_for_TCP(TCP_HEADER,src_address,dst_address,ts_sec,ts_usec,incl_len,IP_header_len,total_length):
    ...
    payload_list.append((connection_4_tuple,payload,ack,seq))
...
def get_start_and_end_time(connections):
    ...
    for item in SYN_list:
        if (connections == item[0] and item[2] == 1) or (combination == item[0] and item[2] == 1):
    ...
    for item in FIN_list:
        if (connections == item[0] and item[2] == 1) or (combination == item[0] and item[2] == 1):
            last_occurance = len(FIN_list) - FIN_list[::-1].index(item) - 1
            break
    

    start = SYN_list[first_occurance][1]
    end = FIN_list[last_occurance][1]
...
def print_output():
    ...
    print("\nMinimum RTT value:","\nMean RTT value:","\nMaximum RTT value:")
    print("\nMinimum number of packets including both sent/received:",min(packet_counter),"\nMean number of packets including both sent/received:",mean(packet_counter),"\nMaximum number of packets including both sent/received:",max(packet_counter))
    print("\nMinimum receive window size including both send/received:",min(complete_connections_window),"\nMean receive window size including both send/received:",round(mean(complete_connections_window),6),"\nMaximum receive window size including both send/received:",max(complete_connections_window))
    print("_"*100,"\n")
