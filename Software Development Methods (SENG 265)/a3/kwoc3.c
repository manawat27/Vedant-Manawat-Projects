...
//Defining structs which will be used to store file lines dynamically
...
//Forward declarations of all the functions used in this program
void command_line_input(int argc, char *argv[], char **input_file, char **exp_filename);
void free_linked_list(node_t **words);
...
//Main function where the variables are initialized and the function calls are made
...
//Function to get the filename and exp_filename from the command line
...
//Function to free every node in the linked list
void free_linked_list(node_t **words){
	node_t *temp = NULL;
	node_t *curr = NULL;
...
//function to open the file, read line by line and call the function to store it dynamically
void read_file_in_array(char *filename, struct Ntab *ntab){
	FILE *fptr;
	char *line = NULL;
	size_t len = 0;
...
//Function that uses dynamic memory to store the lines into an array
...
//Function to tokenize the array of lines and store them into a linked list word by word
//Similar logic used in kwoc1 was used here
void store_words(struct Ntab *ntab, node_t **words, int *word_counter){
	int buff_line_num = 0;
...
//Function to extract the keywords from the linked list made by the previous function
//This function calls another function "remove_word"
void remove_exceptions(node_t **words, node_t **exp_words, int *word_counter){
	node_t *curr;
	node_t *curr2;
...
//Function to actually delete the node from the linked list
node_t *remove_word(node_t *words, char *text){
	node_t *curr, *prev;
	prev = NULL;
...
//Function that will remove any duplicate keywords from the linked list
//Idea adpoted from: https://www.geeksforgeeks.org/remove-duplicates-from-a-sorted-linked-list/
void remove_duplicates(node_t **words, int *word_counter){
	node_t *curr = *words;
	node_t *temp;
...
//Idea for converting each word to lower case was adopted from: https://www.tutorialspoint.com/c-program-for-lowercase-to-uppercase-and-vice-versa
int count_words_in_line(char word[MAX_WORD_LEN], char buffer[MAX_LINE_LEN]){
    int word_count = 0;
    int occur_count = 0;
	...
	free_linked_list(&words_in_a_line);
    return(occur_count);
}
